...
Exports JSON documents from collection `documents_pre_bank_2025`
into a single compressed ZIP archive without saving temporary files on disk.
"""
...
class ExporterConfig:
    ...
    collection_name: str = "documents_pre_bank_2025"
    output_zip_path: Path = Path("./documents_pre_bank_2025.zip")
    batch_size: int = 100
...
class MongoZipExporter:
    ...
    def run(self) -> None:
        ...
        logger.info(
            "Found %d document(s). Starting direct-to-ZIP stream export...", total_docs
        )
        ...
        try:
            with zipfile.ZipFile(
                ...
                for idx, doc in enumerate(self._stream_documents(), start=1):
                    ...
                    if idx % 50 == 0 or idx == total_docs:
                        logger.info(
                            "  [%d/%d] Compressed: %s", idx, total_docs, filename
                        )
            ...
            logger.warning(
                "Export interrupted by user. Removing incomplete output archive..."
            )
