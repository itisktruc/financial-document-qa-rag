"""
Tải OCR .txt năm 2025 từ HF → preprocess → lưu Mongo + JSON chuẩn.

- JSON: app/preprocessing/mongo_json/
- Mongo: resume theo _id = repo path
- Download tạm qua tempfile (không giữ _hf_2025_tmp trong project)

python -m preprocessing.hf_to_mongo --db-name financial_rag --collection documents_2025 --limit 1
python -m preprocessing.hf_to_mongo --db-name financial_rag --collection documents_2025
"""
...
def list_txt_2025(repo_id: str = REPO_ID, repo_type: str = REPO_TYPE) -> list[str]:
    api = HfApi()
    all_paths = api.list_repo_files(repo_id=repo_id, repo_type=repo_type)
    txt_2025 = [p for p in all_paths if p.endswith(".txt") and "/2025/" in p]
    logger.info("Found %d .txt file(s) for year 2025 in %s.", len(txt_2025), repo_id)
    return txt_2025
...
def run(
    ...
    for i, repo_path in enumerate(txt_paths, start=1):
        ...
        try:
            ...
            #json_path = save_json(payload, repo_path)
            n_done += 1
            ...
            """
            logger.info(
                "[%d/%d] Saved Mongo + JSON: %s "
                "(pages=%d, blocks=%d, risk=%s) -> %s",
                i,
                n_total,
                repo_path,
                len(document.pages),
...
def main() -> None:
    ...
    parser.add_argument("--collection", default="documents_2025")
